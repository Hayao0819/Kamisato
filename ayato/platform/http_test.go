...
func TestHTTPErrorCodeForStatus(t *testing.T) {
	t.Parallel()
	tests := map[int]HTTPErrorCode{
		http.StatusBadRequest:            HTTPCodeInvalidRequest,
		...
		http.StatusTooManyRequests:       HTTPCodeRateLimited,
		http.StatusInternalServerError:   HTTPCodeInternal,
		http.StatusNotImplemented:        HTTPCodeNotImplemented,
		http.StatusBadGateway:            HTTPCodeUpstream,
		http.StatusServiceUnavailable:    HTTPCodeServiceUnavailable,
		http.StatusTeapot:                HTTPCodeRequestFailed,
	}
	for status, want := range tests {
		status, want := status, want
		t.Run(http.StatusText(status), func(t *testing.T) {
			t.Parallel()
			if got := HTTPErrorCodeForStatus(status); got != want {
				t.Fatalf("HTTPErrorCodeForStatus(%d) = %q, want %q", status, got, want)
			}
		})
	}
}
...
func TestNewHTTPError(t *testing.T) {
	t.Parallel()
	got := NewHTTPError(http.StatusNotFound, "package not found")
	if got.Code != HTTPCodeNotFound || got.Message != "package not found" {
		t.Fatalf("NewHTTPError() = %+v", got)
	}
}
...
func TestOrigin(t *testing.T) {
	tests := []struct {
		...
		{
			raw: "https://example.com/path?q=1", scheme: "https",
			origin: "https://example.com", ok: true,
		},
		{
			raw: "http://example.com:8080/", scheme: "http",
			origin: "http://example.com:8080", ok: true,
		},
		{raw: "/relative"},
		{raw: "://bad"},
		{},
	...
	for _, test := range tests {
		...
		if scheme != test.scheme || origin != test.origin || ok != test.ok {
			t.Errorf(
				...
				test.raw, scheme, origin, ok,
				test.scheme, test.origin, test.ok,
			)
		}
	}
}
