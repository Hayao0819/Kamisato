...
// The approval page hands a pending code off to GitHub OAuth, binding the flow to
// the browser and carrying the user_code inside the signed state.
func TestDeviceApproveRedirectsToGitHub(t *testing.T) {
	h, dev, signer := deviceHandler(t, 42)
	if err := dev.CreateDevice("dc-3", "WXZB-CDFG", deviceCodeTTL); err != nil {
		t.Fatalf("CreateDevice: %v", err)
	}
	r := gin.New()
	r.GET("/approve", h.DeviceApproveHandler)

	// A code typed lowercase and without the dash still resolves.
	w := httptest.NewRecorder()
	r.ServeHTTP(w, httptest.NewRequest(http.MethodGet, "/approve?user_code=wxzbcdfg", nil))
	if w.Code != http.StatusFound {
		t.Fatalf("approve = %d, want 302 (body %s)", w.Code, w.Body.String())
	}
	loc, err := url.Parse(w.Header().Get("Location"))
	if err != nil {
		t.Fatalf("parse Location: %v", err)
	}
	if !strings.Contains(loc.Host, "github.com") {
		t.Fatalf("Location host = %q, want github.com", loc.Host)
	}
	var nonce string
	...
	rec, err := signer.VerifyTyp(loc.Query().Get("state"), auth.TypState)
	if err != nil {
		t.Fatalf("verify state: %v", err)
	}
	if !rec.Device || rec.UserCode != "WXZB-CDFG" || rec.Binding != auth.HashHex(nonce) {
		t.Fatalf("state = %+v, want Device/UserCode=WXZB-CDFG/binding=sha256(cookie)", rec)
	}

	// An unknown code re-renders the form, never redirects.
	w = httptest.NewRecorder()
	r.ServeHTTP(w, httptest.NewRequest(http.MethodGet, "/approve?user_code=ZZZZ-ZZZZ", nil))
	if w.Code != http.StatusNotFound {
		t.Fatalf("unknown approve = %d, want 404", w.Code)
	}
}

// finishDeviceLogin approves the record; denyDeviceLogin rejects it. Exercising
// them directly avoids standing up a fake GitHub, mirroring the CLI loopback test.
func TestFinishAndDenyDeviceLogin(t *testing.T) {
	h, dev, _ := deviceHandler(t, 42)
	if err := dev.CreateDevice("dc-approve", "APPR-OVED", deviceCodeTTL); err != nil {
		t.Fatalf("CreateDevice: %v", err)
	}
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodGet, "/cb", nil)
	h.finishDeviceLogin(c, &auth.Claims{Device: true, UserCode: "APPR-OVED"}, githubUser{ID: 42, Login: "alice"})
	if rec.Code != http.StatusOK {
		t.Fatalf("finishDeviceLogin = %d, want 200", rec.Code)
	}
	if w := pollDevice(t, h, "dc-approve"); w.Code != http.StatusOK {
		t.Fatalf("after approval, poll = %d, want 200 token", w.Code)
	}

	if err := dev.CreateDevice("dc-deny", "DENY-CODE", deviceCodeTTL); err != nil {
		t.Fatalf("CreateDevice: %v", err)
	}
	rec = httptest.NewRecorder()
	c, _ = gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodGet, "/cb", nil)
	h.denyDeviceLogin(c, &auth.Claims{Device: true, UserCode: "DENY-CODE"})
	if w := pollDevice(t, h, "dc-deny"); deviceErr(t, w) != "access_denied" {
		t.Fatalf("after denial, poll = %q, want access_denied", deviceErr(t, w))
	}
}
