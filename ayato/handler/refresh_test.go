...
func TestRefreshIssuesNewAccessAndRotates(t *testing.T) {
	...
	if _, verr := h.signer.VerifyTyp(resp.Token, auth.TypCLI); verr != nil {
		t.Fatalf("issued access token must verify as a CLI token: %v", verr)
	}
	if _, verr := h.signer.VerifyTyp(resp.RefreshToken, auth.TypRefresh); verr != nil {
