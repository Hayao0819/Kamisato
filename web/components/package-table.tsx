...
import { Download, MoreVertical, Info, PackageIcon, Calendar } from "lucide-react"
  ...
  const renderMobileView = () => {
    return (
      <div className="grid grid-cols-1 gap-4">
        {packages.length === 0 ? (
          <div className="text-center p-8 border rounded-md">パッケージが見つかりませんでした</div>
        ) : (
          packages.map((pkg) => (
            <Card key={pkg.id}>
    ...
    return (
      <div className="rounded-md border">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead className="w-[300px]">パッケージ名</TableHead>
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h2 className="text-2xl font-bold tracking-tight">パッケージ一覧</h2>
