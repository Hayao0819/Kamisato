    ...
    <Card className="overflow-hidden">
      <CardHeader className="pb-2">
        <div className="flex justify-between items-center">
          <CardTitle className="text-base sm:text-lg">{server.name}</CardTitle>
        ...
        <div className="grid gap-3 sm:gap-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs sm:text-sm">稼働時間: {server.uptime}</span>
            </div>
            <div className="flex items-center gap-2">
              <RefreshCw className="h-3 w-3 sm:h-4 sm:w-4 text-muted-foreground" />
              <span className="text-xs sm:text-sm">最終同期: {server.lastSync}</span>
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4">
            <div className="flex items-center gap-2">
              <Package className="h-3 w-3 sm:h-4 sm:w-4 text-muted-foreground" />
              <span className="text-xs sm:text-sm">パッケージ数: {server.packages.toLocaleString()}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs sm:text-sm">負荷: {server.load}</span>
            </div>
          </div>
          <div>
            <div className="flex items-center justify-between mb-1">
              <HardDrive className="h-3 w-3 sm:h-4 sm:w-4 text-muted-foreground" />
              <span className="text-xs sm:text-sm">ディスク使用量: {server.diskUsage}%</span>
            </div>
            <Progress value={server.diskUsage} className="h-1.5 sm:h-2" />
          </div>
        </div>
      </CardContent>
    </Card>
