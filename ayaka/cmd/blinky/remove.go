...
// removeCmd removes one or more packages from a Blinky server.
// Usage: ayaka blinky remove [--server URL] [--username USER] [--password PASS] [--ask-pass] <repo> <pkg...>
func removeCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			repoName := args[0]
			pkgs := args[1:]
			if err := client.RemovePackages(repoName, pkgs...); err != nil {
				return err
			}
	...
	cmd.Flags().StringP("server", "s", "", "Server URL to remove packages from (default: serverdb default)")
...
func init() {
	subCmds = append(subCmds, removeCmd())
}
