...
// AddFormatFlags registers the shared --format/--json output flags used by the
// list-like commands, so every one of them speaks the same Docker-style
// templating and offers the same JSON escape hatch for scripting.
func AddFormatFlags(cmd *cobra.Command) {
	cmd.Flags().String("format", "", "Format output with a Go template (Docker-style; 'table ...' or 'json')")
