...
func saraWith(a AyatoSource) *SaraConfig {
	// An explicit TrustStore keeps these per-source checks independent of the
	// ambient user-config-dir guard.
	return &SaraConfig{Port: 10713, TrustStore: "/tmp/sara-test/trust.json", Ayato: []AyatoSource{a}}
}
...
func TestAyatoTrustStoreGuard(t *testing.T) {
	...
	t.Setenv("HOME", "")
	if err := (&SaraConfig{Port: 10713, Ayato: []AyatoSource{src}}).Validate(); err == nil {
		t.Error("ayato + no durable trust_store should be refused")
	}
	// An explicit trust_store satisfies the requirement.
	if err := (&SaraConfig{Port: 10713, TrustStore: "/srv/sara/trust.json", Ayato: []AyatoSource{src}}).Validate(); err != nil {
		t.Errorf("explicit trust_store should be accepted: %v", err)
	}
	// No ayato sources: the guard does not apply.
	if err := (&SaraConfig{Port: 10713}).Validate(); err != nil {
