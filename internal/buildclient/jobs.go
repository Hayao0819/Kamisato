...
func StreamLogs(ctx context.Context, base, token, id string, w io.Writer) error {
	...
	if err != nil {
		return errors.WrapErr(err, "failed to open log stream")
	}
	...
	if !strings.HasPrefix(resp.Header.Get("Content-Type"), "text/event-stream") {
		if _, err := io.Copy(w, resp.Body); err != nil {
			return errors.WrapErr(err, "failed to read logs")
		}
		return nil
	}
	...
	for scanner.Scan() {
		...
		if _, err := fmt.Fprintln(w, data); err != nil {
			return errors.WrapErr(err, "failed to write log line")
		}
	}
	if err := scanner.Err(); err != nil {
		return errors.WrapErr(err, "failed to read log stream")
	}
	return nil
}
