...
func Open(path string) (*Store, error) {
	...
	if err != nil {
		return nil, errors.WrapErr(err, "failed to read trust store")
	}
	if err := json.Unmarshal(raw, &s.data); err != nil {
		return nil, errors.WrapErr(err, "corrupt trust store")
	}
...
func (s *Store) Save() error {
	...
	if err := os.MkdirAll(filepath.Dir(s.path), 0o750); err != nil {
		return errors.WrapErr(err, "failed to create trust store dir")
	}
	...
	if err := os.WriteFile(tmp, raw, 0o600); err != nil {
		return errors.WrapErr(err, "failed to write trust store")
	}
