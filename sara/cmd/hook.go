...
//go:embed sara-verify.hook.tmpl
var hookTemplate string

const (
	defaultHookDir = "/usr/share/libalpm/hooks"
	hookFileName   = "sara-verify.hook"
)

func hookCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "hook",
		Short: "Manage the pacman PreTransaction hook that runs 'sara verify'",
	}
...
func hookInstallCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if err != nil {
				return utils.WrapErr(err, "cannot resolve the sara binary path")
			}
			exec := self + " verify"
			if configPath != "" {
	...
	cmd.Flags().StringVar(&configPath, "config-path", "", "sara config path to bake into the hook's Exec")
	return cmd
}
