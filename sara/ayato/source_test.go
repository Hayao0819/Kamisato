	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case catalogPath:
			if alg == "none" { // legacy/unsigned envelope
				payload, _ := json.Marshal(saraproto.SignedPayload{IssuedAt: time.Now().UTC(), Catalog: testCat})
				_ = json.NewEncoder(w).Encode(saraproto.CatalogEnvelope{Payload: payload, Alg: "none"})
				return
			}
